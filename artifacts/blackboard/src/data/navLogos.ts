...
/**
 * Dock-sized WebP twin of a raster mark under /assets/nodes, generated by
 * scripts/make-blackboard-icons.mjs. The originals are full-size art (up to
 * 1.7 MB) drawn at 24px; SVGs and bundled imports pass through unchanged.
 */
export function dockIcon(src: string): string;
export function dockIcon(src: string | undefined): string | undefined;
export function dockIcon(src: string | undefined): string | undefined {
  const match = src?.match(/^\/assets\/nodes\/([^/?]+)\.(?:png|jpe?g|webp)(\?.*)?$/i);
  return match ? `/assets/nodes/icons/${match[1]}.webp${match[2] ?? ''}` : src;
}
