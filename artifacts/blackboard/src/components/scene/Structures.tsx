...
function Stairs() {
  const mat = useMemo(
    () => makeMat(SEA_SCENE.structures.moss, 0.84, SEA_SCENE.emissive.moss),
    [],
  );
  const specs = useMemo<BlockSpec[]>(() => {
    const rand = mkRand(617);
    const out: BlockSpec[] = [];
    const anchors = [0.1, 0.3, 0.5, 0.7, 0.9];
    anchors.forEach((tt, i) => {
      const pt = curve.getPoint(tt);
      const side = i % 2 === 0 ? -1 : 1;
      ...
      out.push(
        ...buildStairs(cx, cy, cz, 7 + Math.floor(rand() * 4), ang, side),
      );
    });
    return out;
  }, []);
...
function Ships() {
  const mat = useMemo(
    () => makeMat(SEA_SCENE.structures.brown, 0.74, SEA_SCENE.emissive.brown),
    [],
  );
  const specs = useMemo<BlockSpec[]>(() => {
...
function Snakes({ throttle = 4 }: { throttle?: number }) {
  const mat = useMemo(
    () => makeMat(SEA_SCENE.turquoise, 0.86, SEA_SCENE.emissive.teal),
    [],
  );
  ...
  useFrame(({ clock }) => {
    ...
    for (const s of snakes) {
      for (let i = 0; i < SEG; i++) {
        const ang = t * 0.05 * s.speed + s.phase - i * 0.2;
...
function Fish({ throttle = 4 }: { throttle?: number }) {
  const mat = useMemo(
    () => makeMat(SEA_SCENE.gold, 0.82, SEA_SCENE.emissive.brown),
    [],
  );
  const ref = useRef<THREE.InstancedMesh>(null);
  const dummy = useMemo(() => new THREE.Object3D(), []);
  ...
  useFrame(({ clock }, dt) => {
    ...
    for (const f of fish) {
      ...
      if (f.px > 30) f.px = -30;
      else if (f.px < -30) f.px = 30;
      if (f.pz > WORLD_Z_MAX) f.pz = WORLD_Z_MIN;
      else if (f.pz < WORLD_Z_MIN) f.pz = WORLD_Z_MAX;
      const py = f.py + Math.sin(t * 0.6 + f.phase) * 1.2;
      for (let i = 0; i < PARTS; i++) {
        const back = i * 0.6 * f.size;
        const wag =
          Math.sin(t * 6 + f.phase - i * 0.9) * (0.18 + i * 0.1) * f.size;
...
function Monsters({ throttle = 4 }: { throttle?: number }) {
  const mat = useMemo(
    () => makeMat(SEA_SCENE.purple, 0.76, SEA_SCENE.emissive.plum),
    [],
  );
  const ref = useRef<THREE.InstancedMesh>(null);
  const dummy = useMemo(() => new THREE.Object3D(), []);
...
export function Structures({ tier }: { tier: PerformanceTier }) {
  const throttle =
    tier === "high" ? 4 : tier === "medium" ? 5 : tier === "low" ? 6 : 8;
  return (
    <>
      <Bridges />
      <Stairs />
