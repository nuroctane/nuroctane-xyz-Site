...
function SingleBlogNode({
  ...
  useFrame(({ camera: cam, clock }) => {
    ...
    const wobZ =
      Math.cos(elapsed * 0.38 * wobFreq + phase * 1.21) * 0.1 * wobble;
    const floatY =
      Math.sin(elapsed * 0.31 * wobFreq + phase * 0.88) * 0.28 * wobble;
    ...
    group.position.set(
      post.position.x + ox,
      post.position.y + floatY + oy,
      post.position.z + oz,
    );

    _euler.set(
      post.idleRotation.x + wobX,
      post.idleRotation.y,
      post.idleRotation.z + wobZ,
    );
