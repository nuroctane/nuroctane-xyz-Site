...
export function SecondaryOrbit({
  nodeId,
  media,
  centerRef,
  proximityRef,
  ...
  useFrame(({ camera, clock }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const angle =
        (i / count) * Math.PI * 2 + tile.phase + t * params.spinSpeed;
      _ringOffset.set(
        Math.cos(angle) * tile.radius,
        Math.sin(angle) * tile.radius,
        0,
      );
