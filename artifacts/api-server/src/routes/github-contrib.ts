 ...
 * POST /api/github-contrib/refresh  — cron / manual refresh (CRON_SECRET or BOOKS_ADMIN) */
...
async function fetchFromHtml(username: string): Promise<ContribPayload> {
  const response = await fetch(`https://github.com/users/${username}/contributions`, {
    headers: {
      Accept: "text/html",
      "User-Agent": "nuroctane.xyz-contrib-cache",
    },
  });
  ...
  /* Fallback: level-based cells without tooltips */
  if (data.length === 0) {
    let week = 0;
    for (const match of html.matchAll(
      ...
      data.push({
        ...
        week: week++,
        day: d.getUTCDay(),
        count: level, // approximate intensity when exact count unavailable
      });
    }
    /* re-bucket by week index from sorted dates */
...
router.get("/github-contrib", async (_req, res) => {
  try {
    let cached = await kvGet<ContribPayload>(KV_KEY);
    if (!cached || isStale(cached) || !cached.data?.length) {
      try {
        cached = await refreshContributions();
      } catch (err) {
        logger.error({ err }, "Failed to refresh github contrib on GET");
...
/* Vercel Cron invokes GET; manual/admin can POST with password. */
