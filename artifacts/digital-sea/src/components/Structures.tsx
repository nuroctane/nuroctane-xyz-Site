...
function Bridges() {
  ...
  const specs = useMemo<BlockSpec[]>(() => {
    ...
    const out: BlockSpec[] = [];
    // A few cascading bridges near the path, at descending heights
...
function buildStairs(cx: number, cy: number, cz: number, steps: number, ang: number, dir = 1): BlockSpec[] {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const [dx, dz] = rotXZ(lx, 0, ang);
    specs.push({
      x: cx + dx, y: cy + i * stepH, z: cz + dz,
      w: 2.4, h: stepH, d: stepD * 1.02,
      rotY: ang, rotX: 0,
    });
  }
  return specs;
...
function buildShip(cx: number, cy: number, cz: number, ang: number, scale = 1): BlockSpec[] {
  ...
  // hull (two tiers, narrowing downward)
  put(0, 0, 0, 5.4, 0.8, 1.8);
  put(0, -0.7, 0, 4.0, 0.7, 1.2);
  put(0, 0.7, 0, 5.0, 0.5, 1.6);    // deck
  // cabin
  put(-1.0, 1.4, 0, 1.6, 1.0, 1.0);
  // mast + spar (sail rig)
  put(0.8, 2.4, 0, 0.22, 3.2, 0.22);
  put(0.8, 3.4, 0, 0.22, 0.22, 2.4); // cross-spar
  put(0.8, 2.8, 0, 1.8, 1.2, 0.08);  // sail
  // bowsprit
...
// ── Animated snakes (undulating chains drifting in slow circles) ───────────────
function Snakes() {
  ...
  useFrame(({ clock }) => {
    ...
    for (const s of snakes) {
      for (let i = 0; i < SEG; i++) {
        ...
        const by = s.cy
          + Math.sin(t * 0.2 + s.phase - i * 0.3) * s.bob
          + Math.sin(t * 1.6 - i * 0.6) * 0.3;
...
// ── Animated fish (blocky bodies swimming, tails wagging) ─────────────────────
function Fish() {
  ...
  useFrame(({ clock }, dt) => {
    ...
    for (const f of fish) {
      ...
      for (let i = 0; i < PARTS; i++) {
        // segment trails behind heading; tail wags
...
// ── Animated geometric monsters (blocky beings bobbing & turning) ─────────────
// local block layout: core + 4 limbs + crown
const MONSTER_PARTS: [number, number, number, number, number, number][] = [
  // lx, ly, lz, w, h, d
  [0, 0, 0, 1.8, 1.8, 1.8],     // core
  [-1.4, -0.2, 0, 0.7, 1.6, 0.7], // left limb
  [1.4, -0.2, 0, 0.7, 1.6, 0.7],  // right limb
  [0, -0.2, -1.4, 0.7, 1.4, 0.7], // back limb
  [0, -0.2, 1.4, 0.7, 1.4, 0.7],  // front limb
  [0, 1.5, 0, 1.0, 1.0, 1.0],     // crown
  [0, 1.5, 0.7, 0.4, 0.4, 0.4],   // eye nub
];
