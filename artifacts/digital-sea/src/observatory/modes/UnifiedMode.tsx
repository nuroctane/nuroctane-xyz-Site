...
export function UnifiedMode() {
  ...
  const { observer, setObserver, layers, chart, enabledBodies, selectedPlanet, setSelectedPlanet, setSelectedMission } = useObservatory();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Base Esri World Imagery — Google Earth grade, level 19
        const esri = new Cesium.UrlTemplateImageryProvider({
          url: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
          credit: 'Esri, Maxar, NASA, USGS, ESA — base below all',
          maximumLevel: 19,
        });
        ...
        // Add base imagery below all
        (viewer as any).imageryLayers.addImageryProvider(esri);

        viewerRef.current = viewer;
        setStatus('');

        // Smooth initial flight: from high orbit into observer
        try {
          viewer.camera.setView({
            destination: Cesium.Cartesian3.fromDegrees(observer.lon, observer.lat, 25_000_000),
          });
          viewer.camera.flyTo({
            destination: Cesium.Cartesian3.fromDegrees(observer.lon, observer.lat, 8_000_000),
            duration: 2.2,
            easingFunction: Cesium.EasingFunction.QUADRATIC_OUT,
          });
        } catch {}

        // Interaction: double-click sets observer, click selects planet/mission
        ...
        console.error('[Cesium Unified] failed', e);
        setStatus(`Cesium failed to load: ${e?.message ?? String(e)}. Solar system below remains functional.`);
      }
    })();
          ...
          try {
            ...
            for (const q of quakes.slice(0, 200)) {
              ...
              viewer.entities.add({
                ...
                description: `${q.place}<br/>M${mag.toFixed(1)} depth ${q.depth}km<br/>${new Date(q.time).toUTCString()}`,
              });
            }
            setMetStatus(`${quakes.length} quakes USGS`);
          } catch {
            setMetStatus('Quakes unavailable — USGS down, retry later');
          }
        } else {
          for (const e of [...viewer.entities.values]) if ((e.id ?? '').startsWith('quake-')) viewer.entities.remove(e);
        }
        ...
        if (layers.eonet || layers.storms || layers.wildfires || layers.volcanoes) {
          ...
          try {
            ...
            for (const ev of filtered.slice(0, 120)) {
              ...
              viewer.entities.add({ id: `eonet-${ev.id}`, position: Cesium.Cartesian3.fromDegrees(ev.lon, ev.lat, 12000), point: { pixelSize: 9, color, outlineColor: Cesium.Color.BLACK, outlineWidth: 1, disableDepthTestDistance: Number.POSITIVE_INFINITY }, description: `<b>${ev.title}</b><br/>${ev.category}<br/>${ev.date}` });
            }
            setMetStatus((s) => `${s} | ${filtered.length} EONET`);
          } catch {
            setMetStatus((s) => `${s} | EONET unavailable`);
          }
        } else {
          for (const e of [...viewer.entities.values]) if ((e.id ?? '').startsWith('eonet-')) viewer.entities.remove(e);
        }
        ...
        if (layers.winds) {
          ...
          try {
            ...
            for (let i = 0; i < winds.length; i++) {
              ...
              viewer.entities.add({ id: `wind-${i}`, polyline: { positions: Cesium.Cartesian3.fromDegreesArrayHeights([w.lon, w.lat, 8000, lon2, lat2, 8000]), width: Math.min(3, Math.max(1, speed / 6)), material: speed > 12 ? Cesium.Color.CYAN.withAlpha(0.9) : Cesium.Color.WHITE.withAlpha(0.4) } });
            }
          } catch {}
        } else {
          for (const e of [...viewer.entities.values]) if ((e.id ?? '').startsWith('wind-')) viewer.entities.remove(e);
        }
      } catch (e) { console.warn('layers effect', e); }
    })();
  ...
  // Planets as sky entities — reactive
  useEffect(() => {
    const viewer = viewerRef.current;
    if (!viewer) return;
    (async () => {
      ...
      const DIST = 48_000_000; // 48k km beyond Earth limb
      for (const p of chart.planets) {
        ...
        const size = p.id === 'Sun' ? 14 : p.id === 'Moon' ? 10 : p.id === 'Jupiter' || p.id === 'Saturn' ? 8 : p.group === 'outer' ? 7 : 6;
        const color = Cesium.Color.fromCssColorString(p.color);
        viewer.entities.add({
          id: `planet-sky-${p.id}`,
          position: pos,
          point: { pixelSize: isSel ? size + 5 : size, color: color.withAlpha(0.95), outlineColor: Cesium.Color.WHITE.withAlpha(isSel ? 0.9 : 0), outlineWidth: isSel ? 2 : 0, disableDepthTestDistance: Number.POSITIVE_INFINITY },
      ...
      for (const cons of CONSTELLATIONS) {
        for (let li = 0; li < cons.lines.length; li++) {
          ...
          viewer.entities.add({
            id,
            polyline: {
              positions: [new Cesium.Cartesian3(va.x * DIST, va.y * DIST, va.z * DIST), new Cesium.Cartesian3(vb.x * DIST, vb.y * DIST, vb.z * DIST)],
              width: 1,
    ...
    (async () => {
      ...
      if (layers.cities) {
        for (const c of MAJOR_CITIES.slice(0, 50)) {
          ...
          viewer.entities.add({
            id: cid,
            position: Cesium.Cartesian3.fromDegrees(c.lon, c.lat, 1000),
            point: { pixelSize: 2.5, color: Cesium.Color.WHITE.withAlpha(0.55), disableDepthTestDistance: Number.POSITIVE_INFINITY },
            label: { text: c.name, font: '10px Inter, sans-serif', fillColor: Cesium.Color.WHITE.withAlpha(0.6), pixelOffset: new Cesium.Cartesian2(6, -6), distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0, 6_000_000) } as any,
          });
        }
      }
      ...
      for (const m of MISSIONS) {
        ...
        viewer.entities.add({
          id,
          position: pos,
          point: { pixelSize: 7, color: Cesium.Color.fromCssColorString(m.color), outlineColor: Cesium.Color.WHITE, outlineWidth: 1, disableDepthTestDistance: Number.POSITIVE_INFINITY },
          description: `${m.name}<br/>${m.summary}<br/>${m.agency}`,
        });
  ...
  useEffect(() => {
    ...
    const ensureIss = async (lat: number, lon: number) => {
      ...
      if (!e) {
        e = viewer.entities.add({
          id: 'sat-iss',
          position: pos,
          point: { pixelSize: 11, color: Cesium.Color.LIME, outlineColor: Cesium.Color.WHITE, outlineWidth: 1.5, disableDepthTestDistance: Number.POSITIVE_INFINITY },
          label: { text: 'ISS', font: '12px JetBrains Mono, monospace', pixelOffset: new Cesium.Cartesian2(12, 0), disableDepthTestDistance: Number.POSITIVE_INFINITY } as any,
        });
    ...
    const tick = async () => {
      try {
        ...
        await ensureIss(51.6 * Math.sin(t * 0.001), ((t * 0.06) % 360) - 180);
      }
    };
    tick();
      ...
      <div className="obs-unified-overlay">
        <div className="obs-earth-tools obs-earth-tools--unified">
          <div className="obs-earth-search">
            ...
            {hits.length > 0 && (
              <div className="obs-city-hits">
                {hits.map((h, i) => (
                  <button key={i} type="button" onClick={() => flyTo(h.lat, h.lon)}>{h.name.slice(0, 84)}</button>
                ))}
              </div>
            )}
            <div className="obs-earth-note">
              <strong>Earth • Satellite layer above meteo</strong> — Esri 19, double-click globe to set observer (houses & AC).<br />
          ...
          <div className="obs-earth-legend">
            <span className="legend-dot" style={{ background: '#f87171' }} /> Quakes
            <span className="legend-dot" style={{ background: '#22d3ee' }} /> Storms
            <span className="legend-dot" style={{ background: '#fb923c' }} /> Fires
            <span className="legend-dot" style={{ background: '#a3e635' }} /> ISS
          </div>
        </div>

        <div className="obs-solar-mini">
          <div className="obs-solar-mini-hd">
            <span>SOLAR SYSTEM — 3D models (reactive: {chart.zodiac}/{chart.houseSystem} {chart.ayanamsa.toFixed(3)}°)</span>
            <span className="obs-pill" style={{ marginLeft: 6 }}>{chart.planets.length} bodies</span>
          </div>
          <div className="obs-solar-mini-canvas">
            <Suspense fallback={<div className="obs-loading">Loading solar models…</div>}>
              <SolarMini />
            </Suspense>
          </div>
        </div>
